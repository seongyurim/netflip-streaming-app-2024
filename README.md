...
## 📍프로젝트 소개
TMDB 데이터를 활용하여 제작한 넷플릭스 스타일의 영화 소개 앱입니다. 영화를 테마별, 장르별 혹은 제목으로 검색하여 취향에 맞는 작품을 탐색할 수 있습니다. 영화의 상세정보, 예고편, 리뷰 등 다양한 컨텐츠를 사용자에게 제공합니다.
...
- **영화 리스트 슬라이드**: 여러 테마의 영화들을 슬라이드로 제공하여 직관적으로 탐색할 수 있습니다.
- **영화 상세 정보 조회**: 영화 카드를 클릭하면 영화의 상세 정보를 확인할 수 있으며, 비슷한 작품과 리뷰도 함께 제공됩니다.
- **유튜브 예고편 시청**: 선택한 영화의 예고편을 모달 창을 통해 유튜브에서 직접 시청할 수 있습니다.
...
### 6) 기타 컴포넌트
- LoadingSpinner: 로딩스피너를 keyframes를 이용하여 직접 구현합니다.
- Footer: 상업용 페이지의 푸터를 참고하여 레이아웃을 설계하고 반응형으로 구현합니다.

## 📍리액트 쿼리(React Query)
### 1) 의미
- 리액트 쿼리는 서버 상태를 관리하는 라이브러리입니다.
- 서버에서 데이터를 가져오고 관리하는 과정을 단순화하고 자동화해주는 도구입니다.
- 클라이언트에서 서버 데이터와 관련된 작업(데이터 패칭, 캐싱, 동기화, 업데이트)을 효율적으로 수행할 수 있도록 돕습니다.
- 리액트 컴포넌트에서 서버 데이터를 다루는 과정을 더욱 간편하게 만듭니다.

### 2) 이점
- 리액트 쿼리의 가장 큰 장점은 데이터 관리의 효율성입니다.
- 데이터 캐싱을 통해 서버 요청을 최소화하고 네트워크 응답 시간을 줄여 성능을 최적화합니다.
- 또한, 자동 리패칭 기능을 통해 데이터의 일관성을 유지할 수 있으며 컴포넌트에서 데이터 패칭 로직을 분리하여 가독성을 높입니다.
- 이 외에도 자동 리트라이 기능을 통해 실패한 요청을 재시도하거나 백그라운드에서 데이터 업데이트를 처리해 사용자에게 최신 정보를 제공할 수 있습니다.

### 3) 데이터 상태와 캐싱
- 리액트 쿼리는 서버에서 가져온 데이터를 자동으로 캐싱하여 클라이언트 상태와 서버 상태 간의 차이를 관리하는 역할을 합니다.
- 예를 들면 사용자가 영화 리스트 페이지에 다시 접속할 때 이미 캐싱된 데이터를 우선적으로 보여주고, 백그라운드에서 최신 데이터를 가져와 업데이트하는 방식으로 데이터 일관성과 성능을 동시에 유지합니다.
- 캐싱된 데이터는 TTL(Time to Live)을 설정하여 유효 기간을 지정할 수 있어, 사용자가 동일한 데이터를 여러 번 요청할 필요가 없게 됩니다.

### 4) 커스텀 훅을 사용하는 이유
- 커스텀 훅을 사용하는 이유는 코드의 재사용성을 높이고, 로직을 컴포넌트에서 분리하여 가독성을 높이기 위함입니다.
- 영화 정보를 가져오는 로직이나 특정 영화의 상세 정보를 패칭하는 로직을 커스텀 훅으로 정의하면 해당 로직이 필요한 여러 컴포넌트에서 중복 코드 없이 재사용할 수 있습니다.
- 이를 통해 코드 유지보수성을 높이고 각 컴포넌트는 자신의 역할에만 집중할 수 있도록 설계할 수 있습니다.
