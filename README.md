...
- 따라서 모든 페이지 컴포넌트는 단일 레이아웃의 하위 라우트로 설정합니다.
- 내비게이션 항목으로는 로고, 홈, 영화목록, 장르1, 장르2, 장르3 링크, 검색 버튼이 있습니다.

#### 1-1) 내비게이션 좌측: 페이지 이동 링크
- 각 링크들을 객체에 담아 링크명과 URL을 지정합니다.
- 사용자가 현재 위치한 내비게이션의 항목에만 CSS를 부여해서 강조합니다.
- 이를 위해 `useLocation` 훅을 통해 가져온 현재 위치와 객체의 URL을 비교하여 불리언 값을 도출해냅니다.
- 
