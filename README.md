...
- 또한 자동 리패칭 기능을 통해 데이터의 일관성을 유지할 수 있습니다.
- 이 데이터 패칭 로직을 컴포넌트에서 분리하여 가독성을 높입니다.
- 이 외에도 자동 리트라이 기능을 통해 실패한 요청을 재시도하거나 백그라운드에서 데이터 업데이트를 처리하여 사용자에게 최신 정보를 제공할 수 있습니다.

### 3) 데이터 상태와 캐싱
- 리액트 쿼리는 서버에서 가져온 모든 데이터를 자동으로 캐싱하여 클라이언트 상태와 서버 상태 간의 차이를 관리합니다.
- 사용자가 영화 리스트 페이지에 다시 접속할 때 이미 캐싱된 데이터를 우선적으로 보여주고, 직후에 백그라운드에서 최신 데이터를 가져와 업데이트하는 방식으로 데이터 일관성과 성능을 동시에 유지합니다.
- 캐싱된 데이터는 TTL(Time to Live)을 설정하여 유효 기간을 지정할 수 있어 사용자가 동일한 데이터를 여러번 요청할 필요가 없습니다.
